...
module.exports = {
  getCateringHistory: asyncHandler(async (req, res) => {
    ...
    const history = await cateringService.getCateringHistoryForTenant(userIdFromToken, kostId);

    if (history.length === 0) {
      return res.status(200).json({
        message: "Anda belum memiliki riwayat pesanan catering di kost ini atau kost manapun.",
        data: [],
      });
    }
...
createCateringOrderAndPayment: asyncHandler(async (req, res) => {
  const userIdFromToken = req.user.user_id;
  const { items, catatan, metode_bayar, reservasi_id, catering_id } = req.body;
  ...
  try {
    ...
    const result = await cateringService.createCateringOrderWithPayment(
      userIdFromToken,
      { items, catatan, metode_bayar, reservasi_id, catering_id },
      buktiBayarFile
    );
